...
class Qleverfile:
    ...
    def all_arguments():
        ...
        index_args["multi_input_json"] = arg(
            ...
            type=str,
            default=None,
            help="JSON to specify multiple input streams, each with a "
            ...
            "`parallel` (parallel parsing for large inputs, if set to "
            "true, all prefix declaration must be at the beginning)",
        )
        ...
        index_args["parallel-parsing"] = arg(
            "--parallel-parsing",
            choices=["true", "false"],
            default="true",
            help="Whether to parse a single input stream in parallel; "
            "if set to true, all prefix declarations must be at "
            "the beginning of the input; overrides the setting with "
            "the same name in the `.settings.json` file",
        )
