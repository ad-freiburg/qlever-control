...
def show_available_config_names():
    ...
    try:
        ...
        if not config_names:
            raise Exception(
                f'Directory "{qleverfiles_dir}" exists, but ' f"contains no Qleverfiles"
            )
    except Exception as e:
        log.error(f'Could not find any Qleverfiles in "{qleverfiles_dir}" ' f"({e})")
        log.info("")
        log.info(
...
class Actions:
    ...
    def __init__(self):
        ...
        for option in [
            ...
            if option in self.config["index"] and option not in self.config["server"]:
    ...
    def show_process_info(self, psutil_process, cmdline_regex, show_heading=True):
        """
    ...
    def action_setup_config(config_name="default"):
        ...
        try:
            ...
            abort_script()
        log.info(
            f'Created Qleverfile for config "{config_name}"' f" in current directory"
        )
        log.info("")
    ...
    def action_show_config(self, only_show=False):
        ...
        for section in self.config.sections():
            print()
            print(f"[{section}]")
            max_option_length = max([len(option) for option in self.config[section]])
    ...
    def action_remove_index(self, only_show=False):
        ...
        # Show the command line.
        self.show(f"Remove index files {', '.join(index_fileglobs)}", only_show)
        if only_show:
            return
    ...
    def action_start(self, only_show=False):
        ...
        if self.alive_check(port):
            raise ActionException(f"QLever server already running on port {port}")
        ...
        for proc in psutil.process_iter():
            ...
            if re.match(cmdline_regex, cmdline):
                ...
                try:
                    ...
                    raise ActionException(
                        f"Could not kill process with PID " f"{pinfo['pid']}: {e}"
                    )
                return
    ...
    def action_restart(self, only_show=False):
        ...
        # Show action description.
        self.show("Stop running server if found, then start new server", only_show)
        if only_show:
            return
    ...
    def action_index_stats(self, only_show=False):
        ...
        try:
            ...
            raise ActionException(f"Could not read log file {log_file_name}: " f"{e}")
        current_line = 0
    ...
    def action_ui(self, only_show=False):
        ...
        self.show(
            "\n".join(
                [docker_rm_cmd, docker_pull_cmd, docker_run_cmd, docker_exec_cmd]
            ),
            only_show,
        )
        if only_show:
        ...
        # Execute them.
        try:
            subprocess.run(docker_rm_cmd, shell=True, stdout=subprocess.DEVNULL)
            subprocess.run(docker_pull_cmd, shell=True, stdout=subprocess.DEVNULL)
            subprocess.run(docker_run_cmd, shell=True, stdout=subprocess.DEVNULL)
            subprocess.run(docker_exec_cmd, shell=True, stdout=subprocess.DEVNULL)
        except subprocess.CalledProcessError as e:
        ...
        log.info(
            "You can log in as QLever UI admin with username and " 'password "demo"'
        )
    ...
    def action_cache_stats_and_settings(self, only_show=False):
        ...
        try:
            ...
            raise ActionException(f"Failed to get cache stats and settings: " f"{e}")
    ...
    def action_clear_cache(self, only_show=False):
        ...
        # Execute it.
        try:
            subprocess.run(clear_cache_cmd, shell=True, stdout=subprocess.DEVNULL)
    ...
    def action_example_queries(self, only_show=False):
        ...
        for description, query in [
            ...
            try:
                ...
                time_seconds = 0.0
                result_size = 0
                result_string = f"{RED}        FAILED{NORMAL}" f" {RED}({e}){NORMAL}"

            # Print description, time, result in tabular form.
            log.debug(query)
            if len(description) > 60:
                description = description[:57] + "..."
            log.info(f"{description:<60}  {time_seconds:6.2f} s  " f"{result_string}")
            count += 1
        ...
        # Print total time.
        log.info("")
        description = f"TOTAL   for {count} " f"{'query' if count == 1 else 'queries'}"
        log.info(
            f"{description:<60}  {total_time_seconds:6.2f} s  "
            f"{total_result_size:>14,}"
        )
        description = f"AVERAGE for {count} " f"{'query' if count == 1 else 'queries'}"
        log.info(
    ...
    def action_memory_profile(self, only_show=False):
        ...
        try:
            ...
            raise ActionException(
                f"Could not obtain information for process " f"with PID {pid} ({e})"
            )
...
def main():
    ...
    if (
        ...
        log.info("")
        log.info(
            f"{BOLD}Hello, I am the OLD qlever script" f" (version {version}){NORMAL}"
        )
        log.info("")
